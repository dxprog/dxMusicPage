...
class DXMP extends Content {
	...
	private static $_awsLocation = 'AWS_PATH';
	private static $_localSongCache = 'PATH_TO_SONGS';
	private static $_localImageCache = 'PATH_TO_IMAGES';
	private static $_awsKey = 'AWS_KEY';
	private static $_awsSecret = 'AWS_SECRET';
	...
	public static function getTopUserTracks($vars) {
		...
		if (isset($vars['user'])) {
			db_Connect();
			$result = db_Query('SELECT c.*, COUNT(1) AS total FROM hits h INNER JOIN content c ON c.content_id = h.content_id WHERE h.hit_user = "' . db_Escape($vars['user']) . '" GROUP BY h.content_id ORDER BY total DESC LIMIT 30');
	 ...
	 */
	function getMostPlayedSongs($vars) {
